...
function PureChatHeader({ chatId, selectedModelId, selectedVisibilityType, messages, setMessages, sessionId, setSessionId }: {
    ...
    return (
        <div className="w-100 flex justify-between">
            ...
            <div className="flex items-center p-4 relative" ref={dropdownRef}>
                {!isLoggedIn ? (
                    <>
                        <Button
                            className="bg-zinc-900 dark:bg-zinc-100 hover:bg-zinc-800 dark:hover:bg-zinc-200 text-zinc-50 dark:text-zinc-900 hidden md:flex py-1.5 px-2 h-fit md:h-[34px] order-4 md:ml-auto mx-2"
                            onClick={() => router.push('/auth/login')}
                        >
                            Login
                        </Button>
                        <Button
                            className="bg-zinc-200 dark:bg-zinc-100 hover:bg-zinc-100 dark:hover:bg-zinc-200 text-zinc-900 dark:text-zinc-900 hidden md:flex py-1.5 px-2 h-fit md:h-[34px] order-4 md:ml-auto"
                    ...
                    <div className="relative">
                        <Button
                            ...
                            className="bg-zinc-800 dark:bg-zinc-200 text-white dark:text-black py-1.5 px-3 h-fit md:h-[34px]"
                        >
                            {name ?? 'User'}
                        </Button>
                        {showDropdown && (
